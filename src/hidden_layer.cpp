...
{
	accelerator->err_info=0;
	accelerator->err_info|=clSetKernelArg(backward_dWB_kernel, 0, sizeof(int),&N_in);
	accelerator->err_info|=clSetKernelArg(backward_dWB_kernel, 1, sizeof(cl_mem),&dW_acc);
	accelerator->err_info|=clSetKernelArg(backward_dWB_kernel, 2, sizeof(cl_mem),&dB_acc);
	accelerator->err_info|=clSetKernelArg(backward_dWB_kernel, 3, sizeof(cl_mem),&in_acc);
	accelerator->err_info|=clSetKernelArg(backward_dWB_kernel, 4, sizeof(cl_mem),&out_acc);
	accelerator->err_info|=clSetKernelArg(backward_dWB_kernel, 5, sizeof(cl_mem),&delta_in_acc);
	//cout<<"set kernel arg result:"<<accelerator->err_info<<endl;	
}
...
{
	accelerator->err_info=0;
	accelerator->err_info|=clSetKernelArg(backward_delta_kernel, 0, sizeof(int),&N_in);
	accelerator->err_info|=clSetKernelArg(backward_delta_kernel, 1, sizeof(int),&N_out);
	accelerator->err_info|=clSetKernelArg(backward_delta_kernel, 2, sizeof(cl_mem),&W_acc);
	accelerator->err_info|=clSetKernelArg(backward_delta_kernel, 3, sizeof(cl_mem),&in_acc);
	accelerator->err_info|=clSetKernelArg(backward_delta_kernel, 4, sizeof(cl_mem),&delta_in_acc);
	accelerator->err_info|=clSetKernelArg(backward_delta_kernel, 5, sizeof(cl_mem),&delta_out_acc);
	//cout<<"set kernel arg result:"<<accelerator->err_info<<endl;	
}
