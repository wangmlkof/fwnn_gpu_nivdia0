...
void forward_layer::prepare_acc()
{
	...
	//forward
	cout<<"create "<<name<<" gpu memory"<<endl;
	W_acc=clCreateBuffer(accelerator->platform,CL_MEM_READ_WRITE,sizeof(W_TYPE)*W_size,NULL,NULL);
	B_acc=clCreateBuffer(accelerator->platform,CL_MEM_READ_WRITE,sizeof(B_TYPE)*B_size,NULL,NULL);
	in_acc=clCreateBuffer(accelerator->platform,CL_MEM_READ_WRITE,sizeof(LAYER_IN_TYPE)*N_in,NULL,NULL);
	out_acc=clCreateBuffer(accelerator->platform,CL_MEM_READ_WRITE,sizeof(LAYER_OUT_TYPE)*N_out,NULL,NULL);
	//backward
	dW_acc=clCreateBuffer(accelerator->platform,CL_MEM_READ_WRITE,sizeof(W_TYPE)*W_size,NULL,NULL);
	dB_acc=clCreateBuffer(accelerator->platform,CL_MEM_READ_WRITE,sizeof(B_TYPE)*B_size,NULL,NULL);
	delta_in_acc=clCreateBuffer(accelerator->platform,CL_MEM_READ_WRITE,sizeof(LAYER_IN_TYPE)*N_out,NULL,NULL);
	delta_out_acc=clCreateBuffer(accelerator->platform,CL_MEM_READ_WRITE,sizeof(LAYER_OUT_TYPE)*N_in,NULL,NULL);

	//create  kenerl and program
	cout<<"create "<<name<<" opencl program"<<endl;
	//forward
	forward_program=CreateProgram(accelerator->platform,accelerator->device,forward_program_name.c_str());
	forward_kernel=clCreateKernel(forward_program,forward_kernel_name.c_str(),NULL);
	//bakcword
	//dWB
	backward_dWB_program=CreateProgram(accelerator->platform,accelerator->device,backward_dWB_program_name.c_str());
	backward_dWB_kernel=clCreateKernel(backward_dWB_program,backward_dWB_kernel_name.c_str(),NULL);
	//delta
	backward_delta_program=CreateProgram(accelerator->platform,accelerator->device,backward_delta_program_name.c_str());
	backward_delta_kernel=clCreateKernel(backward_delta_program,backward_delta_kernel_name.c_str(),NULL);
}
